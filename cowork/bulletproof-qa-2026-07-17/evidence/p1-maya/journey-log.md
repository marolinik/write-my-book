...
## D-13 RE-VERIFY (post-fix, 2026-07-18)

Team-lead directive: prove the dev-editor no longer re-raises a dismissed, memory-backed, non-critical finding on unchanged content, now that the deterministic persist-time suppression (originalText+category match on dismissed findings, critical-severity exception, non-empty-originalText gate — `src/lib/agents/tools.ts:1283-1317`) is live on the dev server.

| id | method | path | status | expected | verdict | notes |
|---|---|---|---|---|---|---|
| d13rv-findings-before | GET | `.../editorial/findings` | 200 | baseline | PASS | 7 findings. Lineage: `25499afe` dismissed, `d0f79766` still pending; both `originalText` byte-identical (313 chars, sha256 prefix `877f24214227adc7`). `api-traces/d13-reverify-findings-before.json` |
| d13rv-memory-before | GET | `/api/memory?bookId=...` | 200 | 1 active row | PASS | `25499afe`-linked row, `source:"conversation"`, `active:true` — unchanged since dismiss. `api-traces/d13-reverify-memory-before.json` |
| d13rv-content-check | GET | `.../chapters/{chapterId}/content` | 200 | unchanged vs last run | PASS | `wordCount:704`, `version:1`, documentId `b08086f9` — matches journey-log exactly; the critiqued passage is verbatim-present. Chapter sha256 `e2e0f62e...` recorded for future runs. `api-traces/d13-reverify-chapter-content-before.json` |
| d13rv-dismiss-d0f79766 | PATCH | `.../findings/d0f79766` | 200 | dismissed | PASS | Pre-fix re-raise dismissed too — suppression now armed against BOTH lineage entries. No new WriterMemory row created by this dismiss (still exactly 1 active). `api-traces/d13-reverify-dismiss-d0f79766.json`, `api-traces/d13-reverify-memory-after-dismiss.json` |
| d13rv-devedit4-start | POST | `.../agent` (dev-edit) | 200, queued | 200 | PASS | Session `4bd0654f-de53-41f9-a86e-17a1a3faa61f`. `transcripts/d13-reverify-dev-edit-4-start.json` |
| d13rv-devedit4-stream | GET | `.../agent/{sessionId}/stream` (SSE) | complete | complete | PASS | 580 events, 147.7s, `final_status:"complete"`, `findingsCreated:1`, `endReason:"natural"`, cost $0.0278. `transcripts/d13-reverify-dev-edit-4-sse-raw.json` |
| d13rv-verdict-check | GET | `.../editorial/findings` | 200 | no new finding matching dismissed lineage originalText+category | **PASS** | 8 findings total = 7 before + exactly 1 new: `f1b35402` (category `show-tell`, severity `suggestion`, originalText 300 chars sha `f01da3a4` — a DIFFERENT passage, different category, no byte or normalized match against either dismissed lineage entry). Zero re-raises. `api-traces/d13-reverify-findings-after.json` |
| d13rv-memory-after | GET | `/api/memory?bookId=...` | 200 | unchanged | PASS | Still exactly 1 active row (`25499afe`-linked). `api-traces/d13-reverify-memory-after-run.json` |

**D-13 verdict: FIXED-VERIFIED (outcome-level).** On byte-identical chapter content, with the WriterMemory row active and both lineage findings dismissed, dev-edit #4 produced zero findings matching the dismissed lineage (`originalText`+`category`, byte- and whitespace-normalized-compared) and still produced 1 genuinely-new finding on a different passage — so no over-suppression either.

**Honesty caveat — the suppression gate itself was not live-triggered this run.** The SSE stream shows only 2 CreateFinding attempts (4 `tool_use` events = 2 attempts × "Using"+"Executing"), both targeting the new show-tell passage; the model never attempted to re-create the dismissed prose critique, so the deterministic gate had nothing to suppress (0 "suppressed" tool-results in stream). The prompt-side FINDING HISTORY AWARENESS rule held on its own this time — consistent with re-raise being stochastic (dev-edit-2 also didn't re-raise; dev-edit-3 did). The deterministic gate is the backstop for the stochastic case and is verified at unit level: `tests/unit/finding-redismiss-suppression.test.ts` 5/5 green (run 2026-07-18 13:35): (a) suppresses identical text+category w/ whitespace-normalization, (b) critical severity passes through, (c) different text passes, (d) empty originalText never suppresses even when a dismissed row is also empty, (e) different chapter passes.

**Gate probe (empty-originalText safety) — confirmed safe, by code trace + unit test:**
- `normalizeOriginalTextForDismissMatch()` (`tools.ts:52-56`) maps null/undefined/whitespace-only → `""` with an explicit "callers MUST treat as non-matching" contract.
- The suppression block (`tools.ts:1290-1294`) is only entered when severity ≠ critical AND normalized new-text is non-empty — empty skips the entire block (including the DB query): no crash, no suppression, no wasted query.
- Prior side is double-guarded: Prisma filter `originalText: { not: null }` (`tools.ts:1301`) plus normalized comparison — a non-null-but-blank prior normalizes to `""` and can never equal the (guaranteed non-empty) new text.
- Unit case (d) proves the both-sides-empty case creates the finding normally.

**NEW DEFECT observed (unnumbered — ID to be assigned by team-lead): CreateFinding crashes with a raw internal TypeError when the model omits `paragraphNumber`.** Dev-edit #4's first CreateFinding attempt (SSE ev#570-573, toolUseId `call-955d8ca6`) omitted both `paragraphNumber` and `alternatives`; the tool returned `Error executing CreateFinding: Cannot read properties of undefined (reading 'normalize')`. Traced: `validateFinding` (`tools.ts:104`) range-checks `input.paragraphNumber < 1 || > paragraphs.length` — both comparisons are `false` for `undefined`, so it slips through; `paragraphs[undefined - 1]` = `paragraphs[NaN]` = `undefined` (`tools.ts:110`); `fuzzyMatch(anchorQuote, undefined)` then throws at `haystack.normalize("NFC")` (`tools.ts:62`). Pre-existing input-validation gap at the tool boundary — NOT introduced by the D-13 fix (the missing-`alternatives` case alone would have been cleanly REJECTED at `tools.ts:139`, and the D-13 normalize helper is undefined-safe). Impact is contained: the executeTool wrapper caught the throw, surfaced it as a tool error, and the model self-recovered on attempt 2 (full input, finding created, run completed naturally) — but (a) a raw internal error leaks to the LLM instead of a corrective REJECTED message, (b) this failure class bypasses the rejected-row analytics recording (`tools.ts:1240`), and (c) a weaker model might not recover, silently degrading a paid run. Suggested class: S3. Evidence: `transcripts/d13-reverify-dev-edit-4-sse-raw.json` events 570-577 (crashed attempt's full toolInput + successful retry's full toolInput captured verbatim in-stream).
