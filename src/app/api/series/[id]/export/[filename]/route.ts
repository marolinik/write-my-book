...
/** GET /api/series/:id/export/:filename — download an exported series omnibus file. */
export async function GET(_req: NextRequest, { params }: RouteParams) {
  try {
    const user = await requireUser();
    const { id: seriesId, filename } = await params;
    ...
    const storage = getSeriesStorage(user.id, seriesId);
    ...
    console.error("GET /api/series/:id/export/:filename error:", error);
