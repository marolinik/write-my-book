...
## Landing log — 2026-07-19 (session cont.)

- **909c6d9** fix(continuity): RC-4 honest extraction-failure economics (W-D fix 5). Closes the D7=3.0 "failure states lie" crater — no billed-as-success, per-content-version cap (MAX=5, pre-LLM), honest 4-state envelope, new read-only GET/DELETE continuity route, lowYield advisory (never a flag). Two Fable verifiers: econ APPROVE-WITH-NOTES, route/security APPROVE. tsc 0, suite 876/876. Follow-ups -> D-73 (E4 counter-split, E6 post-spend infra window, E7 bible cap) dispatched to d5.
- **88d2cd5** chore(migration): D-71 tenant-isolation edge+node userId backfill (RC-6). userGuard is null-safe, so unstamped legacy data = allow-all; two dry-run/gated scripts restore isolation. node 310/310, edge 340 backfills (0 deletes today). Both Fable-verified APPROVE-WITH-NOTES; H1/H2/H3 hardenings folded (inert on 0-merge census). --execute GATED (backup + quiesce + nodes-first, dev then prod). D-72 follow-up: flip userGuard null-safe -> strict after backfills land in prod.

**Defects registered this session:** D-71 (node isolation, HIGH — remediated by 88d2cd5, awaiting gated --execute), D-72 (strict-guard flip, MED, gated on prod --execute), D-73 (RC-4 follow-up cluster: E4/E6/E7, in progress w/ d5). Next free ID: D-74.

**Unpushed:** 909c6d9, 88d2cd5, + this docs commit — on qa/bulletproof-2026-07-17, ready for `git push`.
