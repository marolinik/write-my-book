...
function insightTypeBadge(type: string, t: UIStrings) {
  switch (type) {
    case "WARNING":
      return (
        <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">{t.agentUI.insightWarning}</Badge>
      );
    case "SUGGESTION":
      return (
        <Badge className="bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">{t.agentUI.insightSuggestion}</Badge>
      );
    case "FLAG":
      return <Badge variant="destructive">{t.agentUI.insightFlag}</Badge>;
    case "CONSTRAINT":
      return (
        <Badge className="bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400">{t.agentUI.insightConstraint}</Badge>
      );
    default:
    ...
    <Card className="transition-colors hover:bg-muted/50">
      <CardContent className="p-3 space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          {t.agentUI.insightFrom}{" "}
          <span className="font-medium">{insight.sourceAgentType}</span>
          {insight.chapterScope.length > 0 && (
            <span>
              {" "}
              &middot; {t.agentUI.chapterAbbrev} {insight.chapterScope.join(", ")}
            </span>
          )}
        </p>
        ...
        <div className="flex items-center pt-1">
          <Button
            ...
            className="gap-1"
          >
            <X className="h-3.5 w-3.5" />{t.editorUI.dismiss}</Button>
        </div>
      </CardContent>
    </Card>
  );
...
export function BlackboardPanel({ bookId }: { bookId: string }) {
  ...
  if (isLoading) {
    return (
      <div className="p-4 text-sm text-muted-foreground">{t.agentUI.loadingInsights}</div>
    );
  }
  ...
  if (insights.length === 0) {
    return (
      <div className="p-4 text-sm text-muted-foreground">{t.agentUI.noInsights}</div>
    );
  }
