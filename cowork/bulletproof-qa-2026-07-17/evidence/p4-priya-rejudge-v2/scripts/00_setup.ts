/**
 * P4 v2 setup + D-20 (chapter-collision) LIVE re-confirm + chapter seeding.
 * No LLM spend. Writes _state.json for the batch scripts.
 *
 * D-20 CONFIRM: book-create auto-creates a placeholder Chapter 1; then
 * POST /api/books/{id}/chapters {chapterNumber:1} must now return a clean 409
 * (v1 baseline: raw 500 {"error":"Failed to create chapter"}).
 */
...
async function main(): Promise<void> {
  console.log("=== P4 v2 setup + D-20 re-confirm ===");
  ...
  const bookName = `Priya Rejudge v2 ${new Date().toISOString().replace(/[:.]/g, "-")}`;
  ...
  // ── D-20 CONFIRM: POST chapter#1 collides with the auto-created placeholder ──
  const collide = await api("POST", `/api/books/${bookId}/chapters`, {
    actNumber: 1,
    chapterNumber: 1,
    title: "Collision probe",
  });
  dump("02_d20_chapter1_collision", {
    note:
      "POST chapterNumber:1 collides with book-create's auto-placeholder chapter 1. " +
      "v1 baseline: raw 500 {error:'Failed to create chapter'}. " +
      "CONFIRM: fix maps Prisma P2002 -> 409.",
    expected_status: 409,
    request: { actNumber: 1, chapterNumber: 1, title: "Collision probe" },
    response: collide,
  });
  console.log(
    `D-20 POST chapter#1 collision -> ${collide.status} (expect 409) : ${JSON.stringify(collide.json ?? collide.text)}`
  );
  ...
  seedResults.push({ chapter: 1, action: "put-content", status: c1.status });
  ...
  for (const n of [2, 3]) {
    ...
    if (chId) {
      ...
      seedResults.push({ chapter: n, action: "put-content", status: cc.status });
    }
  }
  ...
  dump("03_chapters_after_seed", { status: chaptersAfter.status, count: chCount });
